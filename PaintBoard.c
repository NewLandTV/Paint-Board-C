...
int get_input(WORD* vkey, COORD* pos)
{
	...
	{
	case MOUSE_EVENT:
		if (pos && (input_record.Event.MouseEvent.dwButtonState & FROM_LEFT_1ST_BUTTON_PRESSED))	// 마우스 왼쪽 버튼 눌림
		{
		...
		else if (pos && (input_record.Event.MouseEvent.dwButtonState & RIGHTMOST_BUTTON_PRESSED))	// 마우스 오른쪽 버튼 눌림
		{
			CONSOLE_SCREEN_BUFFER_INFO csbi;
...
char getchr(int x, int y)	// 그 좌표에 들어있는 문자를 출력
{
	COORD pos = { x, y };
	TCHAR ch;
	DWORD rd;
		...
		if (be_input())
		{
			if (get_input(&key, &pos) != 0)	// 마우스 클릭할수있도록 만들어진 함수
			{
				...
				if (x >= 0 && x <= 6)	// 도구상자 범위
				{
					if (y >= 15 && y <= 16)	// RESET
					{
						count_x = 0; count_y = 0; count_font = 0, count_color = 0;
					...
					else if (y >= 17 && y <= 18)	// SAVE
					{
						...
						paintboard_save = fopen("C:\\NewLand\\PaintBoard\\paintboard_save", "w");
					...
					else if (y >= 21 && y <= 22 && check_help == 0) // Help on
					{
						...
						WHITE;
						printf("그림판을 지워줍니다. 이때 저장되어있던 것 또한 초기화 됩니다.");
						gotoxy(11, 17);
						...
						WHITE;
						printf("그림판에 그렸는 그림을 text파일로 저장해 줍니다.");
						gotoxy(11, 19);
						...
						WHITE;
						printf("지금까지 그렸던 장면을 처음부터 다시 보여줍니다.");
						gotoxy(11, 21);
